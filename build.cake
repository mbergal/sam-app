...
var commitSha = Argument<string>("commitSha", null);

var version = CalculateVersion(commitSha);

string CalculateVersion(string commitSha) {
    var now = DateTime.Now;
    var year = now.Year;
    var month = now.Month + 1;
    var day = now.Day;
    return $"{year}.{month}.{day}" + (commitSha != null ? $"-{commitSha}": "");
}
...
Task("Run-Local")
	...
	.Does(() => 
    {
        ...
        { 
            Arguments = "local invoke \"HelloWorldFunction\" -e event.json",
        };
